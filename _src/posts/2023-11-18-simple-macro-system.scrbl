...
If you like to make abstractions to avoid repeating yourself, you've likely hit this sort of wall before. You want to make some abstraction, but functions only allow you to abstract over values, not syntax, which isn't enough in a case like this.
...
That would work, but it sounds very messy. We'd need to make a parser for Racket programs, a parser for our special comments, and we'd have to implement all of these rewrite rules in the pre-processor. What if this pre-processor was integrated into the language itself? After all, Racket already has a parser for Racket programs!
...
The loop @racket[(while condition body ...)] will generally look like this:
...
This is pretty cool, but it's just scratching the surface of what macros are capable of. We can implement entire domain specific languages with macros like @racket[match] and @racket[class], and we can even embed entire programming languages like mini kanren.

@;TODO link mini kanren
...
In our little language, macros will be functions that take in s-expressions and return s-expressions. We'll call these functions transformers. Like in Racket, macro usages will look just like function calls. But when a function call expression is a macro usage, instead of evaluating the function and arguments and passing them to the function, we'll pass the whole macro usage expression to the transformer function.
...
For now, we'll start out by making an inerpreter for a language with just variables, constants like numbers, @racket[lambda] expressions, and function applications:

@repl[
(define (eval expr env)
  (match expr
    [(? symbol? x) (hash-ref env x (lambda () (error 'eval "unbound variable ~a" x)))]
    [`(lambda ,argnames ,body)
     (lambda args
       (unless (= (length argnames) (length args)) (error 'eval "arity error"))
       (define arg-env (for/hasheq ([argname argnames] [arg args]) (values argname arg)))
       (eval body (hash-union env arg-env)))]
    [(cons fun-expr arg-exprs)
     (apply (eval fun-expr env) (for/list ([arg-expr arg-exprs]) (eval arg-expr env)))]
    [atom atom]))
(define (eval-top expr) (eval expr (hasheq)))
(eval-top 1)
...
(define (eval expr env)
  (match expr
    ...
    [`(if ,condition-expr ,then-expr ,else-expr)
     (if (eval condition-expr env)
         (eval then-expr env)
         (eval else-expr env))]
    [`(let ([,x ,rhs]) ,body)
     (eval body (hash-set env x (eval rhs env)))]
    [(cons 'begin exprs)
     (last (cons (void) (for/list ([expr exprs]) (eval expr env))))]
    [`(quote ,expr) expr]
    [(cons fun-expr arg-exprs)
     (apply (eval fun-expr env) (for/list ([arg-expr arg-exprs]) (eval arg-expr env)))]
    [atom atom]))
(define (eval-top expr) (eval expr built-ins))
...
@racket[if], @racket[let], and @racket[begin] are unsurprising. @racket[quote] is subtle. First of all the pattern @racket[`',expr] is the same as @code{`(quote ,expr)}. The racket parser (technically, the reader) translates @literal{'expr} into @literal{(quote expr)} before macros expand or code runs (macro expansion refers to macro usages being re-written). So even in our little language, we can use the apostrophe shorthand like in the last example. Anyway, instead of evaluating @racket[expr], we just return it as-is.
...
(define (eval-quasiquote expr env)
  (match expr
    [(list 'unquote expr) (eval expr env)]
...
(define (eval expr env)
  (match expr
    ...
    [`(let-macro ([,x ,transformer-expr]) ,body)
     (eval body (hash-set env x (transformer (eval transformer-expr env))))]
    [(cons fun-expr arg-exprs)
     (define fun (eval fun-expr env))
     (if (transformer? fun)
         (eval ((transformer-fun fun) expr) env)
         (apply fun (for/list ([arg-expr arg-exprs]) (eval arg-expr env))))]
    [atom atom]))
...
First, we made a struct for our transformers so we can distinguish between regular functions and macros. Then, all we had to do was modify the application case to check if the function is a transformer or not. If it is a transformer, we apply it to the entire expression, which produces another expression. Finally, we evaluate the resulting expression. And to create macros, we have a different version of @racket[let] called @racket[let-macro] that turns the value into a transformer before binding it to the variable. That's it! Now we have macros.
